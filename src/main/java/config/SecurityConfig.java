...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable().authorizeRequests()//
				...
				.antMatchers(HttpMethod.POST, Uris.SERVLET_MAP + Uris.PLAYERS).hasRole(Role.PLAYER.name())
				.antMatchers(HttpMethod.DELETE, Uris.SERVLET_MAP + Uris.PLAYERS).hasRole(Role.TRAINER.name())
				.antMatchers(HttpMethod.POST, Uris.SERVLET_MAP + Uris.RESERVES).hasAnyRole(Role.TRAINER.name(),Role.PLAYER.name())
